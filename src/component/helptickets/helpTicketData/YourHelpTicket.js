  ...
  const loginData = useSelector((store) => store.loginLogout.items)
    ...
    useEffect(() => {
        const callHelpTicketPage = async () => {
          try {
            const respose = await fetch(`${BASE_URL}/yourhelpticket`,{
              method: "GET",
              headers: {
            ...
            if(respose.status !== 200) {
                ...
                console.log(`Data is not coming from your helpticket`)
            }
  ...
  (
        <div className='top75'>
            <ThemeProvider theme={defaultMaterialTheme}>
                ...
                title="Your Help Tickets" columns={columns} data={tableData} 
