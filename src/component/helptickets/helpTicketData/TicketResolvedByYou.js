  ...
  const loginData = useSelector((store) => store.loginLogout.items)
    ...
    useEffect(() => {
        const callHelpTicketPage = async () => {
          ...
          try {
            const respose = await fetch(`${BASE_URL}/helpticketresolved`,{
              method: "GET",
              headers: {
            ...
            console.log(`This is status in helpticket data ${respose.status}`);
           console.log(data)
           setTableData(data)
...
export default TicketResolvedByYou
