...
const PendingTasks = () => {
    ...
    const callPendingTasks = async () => {
        ...
        try {
          const respose = await fetch(`${BASE_URL}/completedtasks`,{
            method: "GET",
            headers: {
  ...
  const loginData = useSelector((store) => store.loginLogout.items)
    ...
    const [tableData, setTableData] = useState([])
      ...
      const columns = [
        ...
        { title: "Planning Time", field: "planning", editable: 'never',  render: rowData => formatDate(rowData.planning) },
        { title: "Meeting Type", field: "meetingType", editable: 'never' },
        { title: "Actual Time", field: "actual", editable: 'never', sorting: true, defaultSort: "desc", render: rowData => formatDate(rowData.actual), emptyValue: () => <em>_</em> },
  ...
  (
        <div className='top75'>
            <ThemeProvider theme={defaultMaterialTheme}>
                ...
                title="Completed Tasks - Checklist" columns={columns} data={tableData} 
